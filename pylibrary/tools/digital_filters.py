...
def SignalFilter_LPFButter(signal:np.ndarray, LPF:float, samplefreq:float, NPole:int=8):
...
def SignalFilter_HPFButter(signal:np.ndarray, HPF:float, samplefreq:float, NPole:int=8):
    """Filter with Butterworth low pass, using time-causal lfilter
...
def SignalFilter_LPFBessel(
    signal: np.ndarray, LPF: float, samplefreq: float, NPole: int = 8, reduce: bool = False
):
...
def SignalFilter_Bandpass(signal:np.ndarray, HPF:float, LPF:float, samplefreq:float):
...
def NotchFilter(signal: np.ndarray, notchf: list = [60.0], Q: float = 90.0, QScale: bool = True, samplefreq: float = None):
...
def NotchFilterZP(signal: np.ndarray, notchf: list = [60.0], Q: float = 90.0, QScale: bool = True, samplefreq: float = None):
    """
    Zero Phase
    """
    assert samplefreq is not None
...
def downsample(data: np.ndarray, n: int, axis: int = 0, xvals: str = "subsample"):
...
if __name__ == "__main__":
    ...
    fnotch = NotchFilter(fsig, notchf=[60.0], Q=6.0, QScale=False, samplefreq=fs)
