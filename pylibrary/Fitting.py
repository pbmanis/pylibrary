    ...
    def FitRegion(self, whichdata, thisaxis, tdat, ydat, t0=None, t1=None,
        ...
        for block in range(nblock):
            for record in whichdata:
                ...
                #                 approx_grad = True) # , disp=0, iprint=-1)
                elif method == 'openopt' and openoptFound: # use OpenOpt's routines - usually slower, but sometimes they converge better
                    print 'OpenOpt!!!'
                    ...
                    return
                xfit = numpy.arange(t0, t1, (t1 - t0) / 100.) # min(tx), max(tx), (max(tx)-min(tx))/100.0)
