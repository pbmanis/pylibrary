...
def OneWayAnova(dataDict=None, dataLabel='data', mode='parametric'):
    ...
    if mode not in ['parametric', 'nonparametric']:
        raise ValueError('RStats.OneWayAnova: Mode must be either parametric or nonparametric; got %s' % mode)
    if dataDict is None or not isinstance(dataDict, dict) or len(dataDict.keys()) < 2:
        raise ValueError('RStats.OneWayAnova: dataDict must be a dictionary with at least 2 keys')
...
def permutation(data, dataLabel=None, nperm=10000, decimals=4):
    ...
    -------
    (p, n) : tuple
        p value for test (against null hypothesis), and n, number permutations run
    """
